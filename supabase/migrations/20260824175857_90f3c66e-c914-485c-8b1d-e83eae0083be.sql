CREATE OR REPLACE FUNCTION public.hist_locais_votacao(
  p_ano integer, p_turno integer, p_cargo integer, p_candidato text
)
RETURNS TABLE(
  local_key text, nm_local text, nm_municipio text, ds_endereco text,
  lat double precision, lng double precision, votos bigint, total_local bigint, pct numeric
)
LANGUAGE plpgsql
...
REVOKE EXECUTE ON FUNCTION public.hist_locais_votacao(integer,integer,integer,text) FROM anon, public;
GRANT EXECUTE ON FUNCTION public.hist_locais_votacao(integer,integer,integer,text) TO authenticated, service_role;