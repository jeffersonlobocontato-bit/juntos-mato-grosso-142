ALTER TABLE public.ai_documents DROP CONSTRAINT IF EXISTS ai_documents_doc_category_check;
ALTER TABLE public.ai_documents ADD CONSTRAINT ai_documents_doc_category_check CHECK (doc_category = ANY (ARRAY['plano_governo'::text, 'documento_tecnico'::text, 'noticia'::text, 'comprovacao'::text, 'investimento'::text, 'promessa'::text, 'legislacao'::text, 'outro'::text, 'rag_tecnica_marketing_ia'::text]));
